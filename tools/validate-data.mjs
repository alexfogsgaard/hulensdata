...
const companySlugs = new Set(companies.map(row => row.slug));
...
for (const company of companies) {
  ...
  if (!required(company.name) || !required(company.slug)) report.blocker('COMPANY_IDENTITY', 'Virksomhed mangler navn eller slug', context);
...
for (const deal of deals) {
  ...
  if (!deal.company || !companySlugs.has(deal.company.slug)) report.blocker('DEAL_COMPANY', 'Deal mangler gyldig virksomhed', context);
  ...
  if (Boolean(deal.aftale) !== hasAmount) report.blocker('DEAL_OUTCOME', '`aftale` modsiger beloeb_modtaget', context);
...
for (const event of events) {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(event.event_date || '')) report.blocker('EVENT_DATE', `Ugyldig eventdato: ${event.event_date}`, context);
...
for (const event of events) {
  if (!sourcedEvents.has(event.id)) report.warning('EVENT_WITHOUT_SOURCE', 'Event har ingen synlig kilde', `company_events:${event.id}`);
}
