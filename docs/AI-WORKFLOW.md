...
- Den private read-only capture og dens replaygrænse er dokumenteret i [`docs/database-baseline-readonly-capture.md`](database-baseline-readonly-capture.md). Capturequeries skal forblive SELECT-only, og `npm run check:baseline-capture` må ikke omgås ved senere baselinearbejde.
- Det private PostgreSQL 17 schema-dump og katalogdiffen er dokumenteret i [`docs/database-schema-dump-readonly-review.md`](database-schema-dump-readonly-review.md). Rå dump/log må ikke committes; `npm run check:schema-dump-review` og mutationstesten skal forblive filbaserede og uden production-credentials.
- Den sanitiserede project-only baseline-draft er dokumenteret i [`docs/database-project-baseline-draft.md`](database-project-baseline-draft.md). Den er ikke en migration og må ikke replayes, promoveres eller bruges til remote historikafstemning før en separat, unlinked lokal replay-gate. `npm run check:project-baseline-draft` og mutationstesten kræver kun committede filer.
- Den lokale database-replay-gate og dens sikkerhedsfund er dokumenteret i [`docs/database-local-baseline-replay.md`](database-local-baseline-replay.md). Replayresultatet autoriserer ikke production: `rls_auto_enable()` anbefales fjernet før promotion, `deals`-policyrollen skal besluttes, og ACL-filen er kun et lokalt testet udkast. Full-service Supabase er endnu ikke testet.
- Promotion-kandidaten er dokumenteret i [`docs/database-baseline-promotion-candidate.md`](database-baseline-promotion-candidate.md). Den fjerner den overflødige SECURITY DEFINER-funktion, integrerer en lokalt replayet project-only ACL og bevarer den observerede `deals`-policy som en åben produktbeslutning. Kandidaten er ikke en migration, ikke anvendt på production og må ikke bruges til `db push`, remote replay eller migrationshistorikafstemning.
