...
> Read-only vurdering, 2026-07-16, med fase 4A-status 2026-07-20. Kontrollerne reducerer risiko, men er fortsat gates for enhver senere produktionsskrivning.
>
...
|---|---|---|---|---|
| R1 | **Inbox bliver en parallel sandhed.** Supabase er kanonisk, men filforslag kan leve videre efter apply. | M/H | Inbox indeholder kun operations+preconditions og har ingen `applied`-status i 1.0.0; en senere applied-revision kræver approver, after-hash og backup-id. Coverage regenereres. | Arkitektur-review |
...
- Der er ingen gennemført restore rehearsal. Eksporten fra 2026-07-20 mangler migrationslisten og afvises derfor korrekt som ufuldstændig af fase 4A-builderen.
...
Fase 4A har nu schemafiler, syntetiske fixtures, ren filvalidering, snapshot-dry-run, coverage-generator/overlay, NDJSON-prefixkontrol og backupmanifest-verifikation uden credentials eller produktionsskrivning. Næste risikoreduktion er at skabe en komplet dataeksport med migrationsliste og derefter et fuldt recovery-set samt restore rehearsal i et isoleret miljø. Først derefter kan et apply-design overhovedet overvejes.
